...
- Auto-partitioning rules can only be configured at the time of creating the partitioned table, except `table.auto-partition.num-retention`, which can be modified by `ALTER TABLE ... SET` after table creation.
