...
## Virtual Tables 
Apache Fluss 0.9 introduces **Virtual Tables**, a new system-level abstraction that provides access to **metadata**, **change data**, and other system information without storing additional data. Virtual tables are accessed by appending a suffix to the base table name, offering a clean and intuitive way to interact with internal data streams.

The first virtual table type introduced is the **`$changelog` table**, which exposes the raw **changelog stream** of any Fluss table. By simply appending `$changelog` to a table name, users can access a complete **audit trail** of every data modification, including inserts, updates, and deletes, along with rich metadata.
...
Each changelog record includes three metadata columns prepended to the original table columns:
...
- **Primary Key Tables**: Emit the full spectrum of change types (`+I`, `-U`, `+U`, `-D`), enabling precise tracking of state changes.
...
These modes are essential for **Point-in-Time Recovery** and **ML Backtesting**, allowing data scientists to reconstruct historical states and validate model performance against past data.

The `$changelog` virtual table unlocks several real-time use cases:

### Performance: Optimized Network and Storage Efficiency
The `$changelog` format significantly reduces overhead by providing a single row with both before and after context. This approach eliminates the need for expensive **shuffles**, minimizes **network I/O**, and removes the requirement for **stateful buffering** at the compute layer. Unlike traditional methods that require reading and joining separate rows in memory, Fluss provides full context in a single read, ensuring **maximum throughput** and lower resource consumption.

### Real-Time Anomaly and Risk Detection
Fluss enables high-performance fraud detection and risk monitoring by allowing systems to detect suspicious patterns in a single pass. By comparing `before` and `after` states without complex joins or external state maintenance, financial institutions can instantly calculate balance changes or flag unusual withdrawals. This **stateless monitoring** capability allows for immediate triggers and fraud alerts, ensuring that critical anomalies are caught as they happen.

### Compliance, Audit Trails, and AI Reproducibility
For regulatory environments, Fluss provides a complete, atomic audit trail of all data modifications. Every change—such as a user profile update or a transaction status shift—is captured with precision, proving **data lineage** and change attribution. 
...
Storage-level CDC means no extra compute overhead; changes are already materialized, with just 🐿️:
```sql
