...
- **Large-scale stream processing with Apache Flink.** Stateless compute,
  ...
  Delta Joins and the Aggregation Merge Engine. Recovery drops from minutes
  to seconds and compute scales independently of state size.
...
- **Real-time lakehouse.** Fluss is the hot tier; Iceberg, Paimon, or Lance is
  ...
  source of truth with sub-second freshness.
