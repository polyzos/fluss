...
Additionally, update the `fullVersion`, `shortVersion`, `dockerVersion`, and `quickstartFlinkDockerVersion` fields for the `current` version entry to reflect the current release version.
...
```json
{
    ...
    "quickstartFlinkDockerVersion": "2.2-0.9.0-incubating",
    "paimonVersion": "1.3.1",
    "paimonVersionShort": "1.3",
    "released": false
 }
...
The `apache/fluss` image is **built and pushed automatically** by the [Docker CI workflow](https://github.com/apache/fluss/actions/workflows/docker.yaml) when the RC tag is pushed in the next step — no manual action is required for it.

The `apache/fluss-quickstart-flink` image must be built and pushed **manually**, because it downloads Fluss artifacts from a Maven repository and Fluss does not currently publish snapshots to the ASF snapshot repository. For RC builds, use the staging repository URL from step 6 (e.g., `https://repository.apache.org/content/repositories/orgapachefluss-<STAGING_ID>/`).

From the root directory of the source code, run the following commands to build and push the image:

```bash
$ cd docker/quickstart-flink
docker/quickstart-flink $ docker buildx build --push --platform linux/arm64/v8,linux/amd64 \
...
Verify the `apache/fluss-quickstart-flink` RC image is present:
...
Pushing the tag triggers the Docker CI workflow, which automatically builds and pushes `apache/fluss:${RELEASE_VERSION}-rc${RC_NUM}`. Monitor the workflow run at https://github.com/apache/fluss/actions/workflows/docker.yaml and verify the RC image is present once it completes:
- https://hub.docker.com/r/apache/fluss/tags

Then, update the `dockerVersion` and `quickstartFlinkDockerVersion` fields for the current release RC version in the `website/fluss-versions.json` file on the `main` branch:
- `dockerVersion` → `${RELEASE_VERSION}-rc${RC_NUM}` (e.g., `0.9.0-incubating-rc1`)
- `quickstartFlinkDockerVersion` → `2.2-${RELEASE_VERSION}-rc${RC_NUM}` (e.g., `2.2-0.9.0-incubating-rc1`)

This update should be committed and pushed to the `main` branch.

This ensures that the **Quickstart guide** in the documentation references the correct Docker images for the release candidate. This will help developers to easily test the new release candidate using the Quickstart guide.
...
- Updated `dockerVersion` and `quickstartFlinkDockerVersion` in `fluss-versions.json` on `main` branch
...
docker buildx imagetools create apache/fluss-quickstart-flink:2.2-${RELEASE_VERSION}-rc${RC_NUM} --tag apache/fluss-quickstart-flink:2.2-${RELEASE_VERSION}
```

### 6. Update `fluss-versions.json`

Update the `released` field to `true`, `dockerVersion` to `$RELEASE_VERSION`, and `quickstartFlinkDockerVersion` to `2.2-$RELEASE_VERSION` (both without the RC number) for the current release version in the `website/fluss-versions.json` file on the `main` branch. This update should be committed and pushed to the `main` branch.
...
```json
{
  ...
  "quickstartFlinkDockerVersion": "2.2-0.8.0-incubating-rc1", // ==> "2.2-0.8.0-incubating"
  "released": false                                    // ==> true
}
```
