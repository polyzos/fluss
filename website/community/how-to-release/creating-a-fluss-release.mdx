...
### 7. Stage the `apache/fluss-quickstart-flink` RC image
...
The `apache/fluss-quickstart-flink` image, however, must be built **manually** for RC builds because it downloads Fluss artifacts from the Maven staging repository, which is not publicly accessible from CI. Use the staging repository URL from step 6 (e.g., `https://repository.apache.org/content/repositories/orgapachefluss-<STAGING_ID>/`).
...
Pushing the tag triggers the Docker CI workflow, which automatically builds and pushes `apache/fluss:${RELEASE_VERSION}-rc${RC_NUM}`. Monitor the workflow run at https://github.com/apache/fluss/actions/workflows/docker.yaml and verify both RC images are present once it completes:
