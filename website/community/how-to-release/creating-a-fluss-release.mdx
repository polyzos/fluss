...
```json
{
  ...
  "quickstartFlinkDockerVersion": "1.20-latest-snapshot",
  "paimonVersion": "1.3.1",
...
```json
{
    ...
    "quickstartFlinkDockerVersion": "1.20-0.9.0-incubating",
    "paimonVersion": "1.3.1",
    "paimonVersionShort": "1.3",
    "released": false
 }
...
docker/quickstart-flink $ docker buildx build --push --platform linux/arm64/v8,linux/amd64 \
    ...
    --tag apache/fluss-quickstart-flink:1.20-${RELEASE_VERSION}-rc${RC_NUM} .
```
...
- `quickstartFlinkDockerVersion` → `1.20-${RELEASE_VERSION}-rc${RC_NUM}` (e.g., `1.20-0.9.0-incubating-rc1`)
...
Update the `released` field to `true`, `dockerVersion` to `$RELEASE_VERSION`, and `quickstartFlinkDockerVersion` to `1.20-$RELEASE_VERSION` (both without the RC number) for the current release version in the `website/fluss-versions.json` file on the `main` branch. This update should be committed and pushed to the `main` branch.
...
```json
{
  ...
  "quickstartFlinkDockerVersion": "1.20-0.8.0-incubating-rc1", // ==> "1.20-0.8.0-incubating"
  "released": false                                    // ==> true
}
```
